...
| `CODEX_REFRESH_ENDPOINT` | 覆盖刷新 `access_token` 的端点（默认 `https://chatgpt.com/backend-api/auth/refresh`）。仅当 `auth.json` 含 `refresh_token` 时，在收到 `401` 时自动调用。Codex 刷新端点未正式公开，若默认地址失效请设此变量指向正确地址。 |
